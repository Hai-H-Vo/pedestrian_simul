...
# attempt at loss fn for whole trajectory
...
def single_traj_loss_fn(model, pedestrian_idx, positions, other_positions, velocity, other_velocities, dt):
    traj = single_traj_generator(model, pedestrian_idx, positions[0], other_positions, velocity[0], other_velocities, dt)
    return jnp.linalg.norm(traj[:, 1] - velocity) ** 2
    # return jnp.linalg.norm(traj[0] - positions) ** 2 + jnp.linalg.norm(traj[1] - velocity) ** 2
