...
from simulator.utils import normal, numpify_wall
...
goal_point = np.array([goal_point] * N)
...
def force_fn(state):
    ...
    disp_to_goal = goal_point - state.position
    ...
    prim_goal_orient = np.arctan2(unit_disp_to_goal[:, 1], unit_disp_to_goal[:, 0])

    # updates goal direction after passing/approaching goal point:
    goal_orient = np.where(dist_to_goal < 0.2, 0,
                           np.where(np.absolute(prim_goal_orient) > onp.pi / 2, 0, prim_goal_orient))
    ...
    return PedestrianState(ttc_force_tot(state.position, state.velocity, state.radius, displacement, 1.5, 3) +
                           ...
                           None, None, None, None, goal_orient, None)
...
np.savez("pedestrian_evacuation", positions = np_positions, goal_orientations = np_orientations, walls = np_walls, time_step=time_step)
